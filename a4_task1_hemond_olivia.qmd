...
I plotted histograms of each variable in my data to examine their distributions. As a couple were highly skewed (diesel_pm and canopy_cover), I logged those to approximate a more normal distribution, as shown below.

```{r}
# Pivot only the numeric columns. Log the most skewed variables
enviroscreen_long <- enviroscreen_clean %>% 
  mutate(diesel_pm = log(diesel_pm),
         canopy_cover = log(canopy_cover)) %>% 
